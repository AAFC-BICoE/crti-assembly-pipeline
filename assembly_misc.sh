...
run_spades_PE() {
    ...
    [ ! -z $3 ] && prefix=$3
    get_qsub_script
    # SPAdes uses 16 threads by default.
    sed -i 's/-pe orte 1/-pe smp 16/' qsub_script.sh 
    cmd="qsub -N SPAdes_PE -l h=$submit_host -l mem_free=250G qsub_script.sh \"spades.py --careful --pe1-1 $R1 --pe1-2 $R2 -o $prefix\""
    echo $cmd
    eval $cmd
}
